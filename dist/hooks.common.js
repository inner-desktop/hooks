...
var index = (function (initialValue) {
    var counterRef = React.useRef(-1);
    // key 存储器
    var keyList = React.useRef([]);
    // 内部方法
    var setKey = React.useCallback(function (index) {
...
function useResponsive() {
    init();
    var _a = __read(React.useState(info), 2), state = _a[0], setState = _a[1];
    React.useEffect(function () {
...
function useStorageState(storage, key, defaultValue) {
    var _a = __read(React.useState(function () { return getStoredValue(); }), 2), state = _a[0], setState = _a[1];
...
function useToggle(defaultValue, reverseValue) {
    ...
    var _a = __read(React.useState(defaultValue), 2), state = _a[0], setState = _a[1];
    ...
    // 设置默认值
    var setLeft = React.useCallback(function () {
        setState(defaultValue);
    }, [setState]);
    // 设置取反值
    var setRight = React.useCallback(function () {
...
var useBoolean = function (defaultValue) {
    ...
    var setTrue = React.useCallback(function () { return toggle(true); }, [toggle]);
    var setFalse = React.useCallback(function () { return toggle(false); }, [toggle]);
    return {
        state: state,
        toggle: toggle,
        setTrue: setTrue,
...
function useDocumentVisibility() {
    var _a = __read(React.useState(getVisibility()), 2), documentVisibility = _a[0], setDocumentVisibility = _a[1];
    React.useEffect(function () {
...
function useSelections(items, defaultSelected) {
    ...
    var _a = __read(React.useState(defaultSelected), 2), selected = _a[0], setSelected = _a[1];
...
function useThrottleFn(fn, deps, wait) {
    ...
    React.useEffect(function () { return cancel; }, []);
    return {
        run: run,
        cancel: cancel,
    };
...
var index$2 = (function () {
    var _a = __read(React.useState(initState), 2), state = _a[0], setState = _a[1];
    React.useEffect(function () {
...
function useKeyPress(keyFilter, eventHandler, option) {
    ...
    var element = React.useRef();
    var callbackRef = React.useRef(eventHandler);
    callbackRef.current = eventHandler;
    var callbackHandler = React.useCallback(function (event) {
...
var index$3 = (function (options) {
    ...
    var onEnterRef = React.useRef(onEnter);
    onEnterRef.current = onEnter;
    var onLeaveRef = React.useRef(onLeave);
...
function useSet(initialValue) {
    var initialSet = React.useMemo(function () { return (initialValue === undefined ? new Set() : new Set(initialValue)); }, [initialValue]);
    var _a = __read(React.useState(initialSet), 2), set = _a[0], setSet = _a[1];
    var stableActions = React.useMemo(function () { return ({
    ...
    var utils = __assign({ has: React.useCallback(function (key) { return set.has(key); }, [set]) }, stableActions);
    return [set, utils];
}
...
function useMap(initialValue) {
    var initialMap = React.useMemo(function () { return (initialValue === undefined ? new Map() : new Map(initialValue)); }, [initialValue]);
    var _a = __read(React.useState(initialMap), 2), map = _a[0], set = _a[1];
    var stableActions = React.useMemo(function () { return ({
    ...
    var utils = __assign({ get: React.useCallback(function (key) { return map.get(key); }, [map]) }, stableActions);
    return [map, utils];
}
...
function useTextSelection() {
    ...
    var arg = React.useRef(args[0]);
    var ref = React.useRef();
    var _a = __read(React.useState(initState$1), 2), state = _a[0], setState = _a[1];
...
function useCounter(initialValue, options) {
    ...
    var _a = __read(React.useState(init), 2), current = _a[0], setCurrent = _a[1];
...
var useUpdate = function () {
    var _a = __read(React.useState(0), 2), setState = _a[1];
...
var index$4 = (function (initialValue, transformer) {
    ...
    var reset = React.useCallback(function () { return setValue(initialValue); }, [setValue]);
    var transformerRef = React.useRef(transformer);
...
function useHistoryTravel(initialValue) {
    ...
    var updateValue = React.useCallback(function (val) {
        setHistory({
            present: val,
            future: [],
    ...
    var _forward = React.useCallback(function (step) {
    ...
    var _backward = React.useCallback(function (step) {
        if (step === void 0) { step = -1; }
        if (past.length === 0) {
            return;
        }
    ...
    var go = React.useCallback(function (step) {
        if (step === 0) {
            return;
        }
        if (step > 0) {
    ...
    return {
        ...
        go: go,
        back: React.useCallback(function () {
            go(-1);
        }, [go]),
        forward: React.useCallback(function () {
            go(1);
        }, [go])
    };
}
...
function useAsync(service, options) {
    ...
    var newstFetchKey = React.useRef(DEFAULT_KEY);
    // 持久化一些函数
    ...
    var _l = __read(React.useState(function () {
        // 如果有 缓存，则从缓存中读数据
        if (cacheKey) {
    ...
    var fetchesRef = React.useRef(fetches);
    ...
    // 重置 fetches
    var reset = React.useCallback(function (newCacheKey) {
        if (newCacheKey) {
    ...
    var noReady = React.useCallback(function (name) { return function () {
...
function useLoadMore(service, options) {
    ...
    var _c = __read(React.useState(false), 2), loadingMore = _c[0], setLoadingMore = _c[1];
    React.useEffect(function () {
        if (options.fetchKey) {
    ...
    var dataGroup = React.useMemo(function () {
        var listGroup = [];
...
function usePaginated(service, options) {
    ...
    var runChangePaination = React.useCallback(function (paginationParams) {
    ...
    var totalPage = React.useMemo(function () { return Math.ceil(total / pageSize); }, [pageSize, total]);
    ...
    var changeCurrentRef = React.useRef(changeCurrent);
    ...
    // 表格翻页 排序 筛选等
    var changeTable = React.useCallback(function (p, f, s) {
        runChangePaination({
            current: p.current,
...
function useRequest(service, options) {
    if (options === void 0) { options = {}; }
    var contextConfig = React.useContext(ConfigContext);
    ...
    var paginatedRef = React.useRef(paginated);
    var loadMoreRef = React.useRef(loadMore);
